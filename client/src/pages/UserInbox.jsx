...
const UserInbox = ({ loading }) => {
  ...
  const onlineCheck = (chat) => {
    ...
    const online = onlineUsers.find((user) =>
      chatMembers.includes(user.userId)
    );
  ...
  const sendMessageHandler = async (e) => {
    ...
    try {
      if (newMessage !== "") {
        const res = await axios.post(`${server}/message/create`, message);
  ...
  const imageSendingHandler = async (e) => {
    ...
    try {
      const res = await axios.post(`${server}/message/create`, {
        images: e,
        sender: user._id,
        text: newMessage,
        conversationId: currentChat._id,
  ...
  useEffect(() => {
    const handleSeen = async (messageId) => {
      ...
      if (unseenMessages.length > 0) {
        ...
        try {
          await axios.post(`${server}/message/seen/${messageId}`, {
            messageIds: unseenMessageIds,
          });
          socketId.emit("messageSeen", { messageId: unseenMessageIds });
        } catch (error) {
...
const MessageList = ({ data, index, setOpen, setCurrentChat, me, setUserData, userData, online, setActiveStatus, loading }) => {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      try {
        const res = await axios.get(`${server}/shop/get/${userId}`);
  ...
  return (
    <div className={`w-full flex p-3 px-3 ${active === index ? "bg-[#00000010]" : "bg-transparent"}  cursor-pointer`} onClick={(e) => setActive(index) || handleClick(data._id) || setCurrentChat(data) || setUserData(user) || setActiveStatus(online)}>
      <div className="relative">
        <img src={`${user?.avatar}`} alt="" className="w-[50px] h-[50px] rounded-full" />
        {online ? (
      ...
      <div className="w-full flex p-3 items-center justify-between bg-slate-200">
        <div className="flex">
          ...
          <div className="pl-3">
            ...
            <h1>  hey {activeStatus ? "Active Now" : ""}</h1>
          </div>
        </div>
        ...
        {messages && messages.map((item, index) => (
          <div className={`flex w-full my-2 ${item.sender === sellerId ? "justify-end" : "justify-start"}`} ref={scrollRef} key={index}>
            {item.sender !== sellerId && (
              <img src={`${userData?.avatar}`} className="w-[40px] h-[40px] rounded-full mr-3" alt="" />
            )}
            {item.images && (
              <img src={`${item.images?.url}`} className="w-[300px] h-[300px] object-cover rounded-[10px] ml-2 mb-2" />
            )}
            {item.text !== "" && (
              <div>
